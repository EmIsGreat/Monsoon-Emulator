        ...
        egui::Grid::new("nametables_container")
            ...
            .show(ui, |ui| {
                for nametable_index in 0..4 {
                        ...
                        egui::Grid::new(format!("nametable_{nametable_index}"))
                            ...
                            .show(ui, |ui| {
                                for (i, tile) in nametable.iter().enumerate() {
                                    // Calculate tile position in the 32x30
                                    // nametable
                                    let tile_col = i % 32;
                                    let tile_row = i / 32;

                                    // Each attribute byte covers a 4x4 tile
                                    // area (32x32 pixels)
                                    // Attribute table is 8 bytes wide (8 * 4 =
                                    // 32 tiles)
                                    ...
                                    // Each attribute byte contains 4 palette
                                    // indices (2 bits each)
                                    // for the four 2x2 tile quadrants within
                                    // the 4x4 area:
                                    //   bits 0-1: top-left     bits 2-3:
                                    // top-right
                                    //   bits 4-5: bottom-left  bits 6-7:
                                    // bottom-right
                                    // Use bit 1 of tile position to determine
                                    // quadrant
                    ...
                    // End row after every 2 nametables (0,1 on first row, 2,3
                    // on second)
