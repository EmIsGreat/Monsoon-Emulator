...
impl DebugOverlay for TileGridOverlay {
    ...
    fn draw(&self, ctx: &OverlayContext) {
        ...
        for col in 1..cols {
            let x = ctx.rect.left() + (f32::from(col) * f32::from(TILE_SIZE) * ctx.pixel_size.x);
        ...
        for row in 1..rows {
            let y = ctx.rect.top() + (f32::from(row) * f32::from(TILE_SIZE) * ctx.pixel_size.y);
            ctx.painter.line_segment(
                [
...
impl DebugOverlay for ScanlineDotOverlay {
    ...
    fn draw(&self, ctx: &OverlayContext) {
        ...
        if position.scanline < TOTAL_OUTPUT_HEIGHT {
            let y = ctx.rect.top() + (f32::from(position.scanline) * ctx.pixel_size.y);
        ...
        if position.dot < TOTAL_OUTPUT_WIDTH {
            let x = ctx.rect.left() + (f32::from(position.dot) * ctx.pixel_size.x);
        ...
        if position.dot < TOTAL_OUTPUT_WIDTH && position.scanline < TOTAL_OUTPUT_HEIGHT {
            let min = egui::pos2(
                ctx.rect.left() + (f32::from(position.dot) * ctx.pixel_size.x),
                ctx.rect.top() + (f32::from(position.scanline) * ctx.pixel_size.y),
            );
...
pub fn render_debug_overlays(
    ...
    let pixel_size = egui::vec2(
        rect.width() / f32::from(TOTAL_OUTPUT_WIDTH),
        rect.height() / f32::from(TOTAL_OUTPUT_HEIGHT),
    );
...
fn register_u16(registers: &RegisterMap, key: &str) -> Option<u16> {
    match registers.get(key)?.value {
        ...
        RegisterValue::U8(value) => Some(u16::from(value)),
