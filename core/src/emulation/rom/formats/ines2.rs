...
impl RomParser for Ines2 {
    fn get_name(&self) -> &'static str { "NES 2.0" }
    ...
    fn parse(&self, rom: &[u8], name: Option<&String>) -> Result<RomFile, ParseError> {
        ...
        let default_expansion_device = rom[15] & 0b0011_1111;
...
impl Ines2 {
    ...
    pub fn get_prg_rom_size(prg_rom_size_lsb: u16, prg_rom_size_msb: u16) -> u32 {
        if prg_rom_size_msb <= 0xE {
            u32::from((prg_rom_size_msb << 8) | prg_rom_size_lsb) * 16 * 1024
        } else {
            let exponent = u32::from(prg_rom_size_lsb >> 2);
            let mult = (u32::from(prg_rom_size_lsb & 0b11) * 2) + 1;
            (1 << exponent) * mult
        }
    }
    ...
    pub fn get_chr_rom_size(chr_rom_size_lsb: u16, chr_rom_size_msb: u16) -> u32 {
        if chr_rom_size_msb <= 0xE {
            u32::from((chr_rom_size_msb << 8) | chr_rom_size_lsb) * 8 * 1024
        } else {
            let exponent = u32::from(chr_rom_size_lsb >> 2);
            let mult = (u32::from(chr_rom_size_lsb & 0b11) * 2) + 1;
            (1 << exponent) * mult
        }
    }
}
